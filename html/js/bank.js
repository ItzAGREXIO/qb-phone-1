...
QB.Phone.Functions.LoadMetaData = function(MetaData) {
    ...
    if (MetaData.profilepicture == "default") {
        $('.bank-tab-icon').html('<img src="./img/default.png">');
    } else {
        $('.bank-tab-icon').html('<img src="'+MetaData.profilepicture+'">');
    }
...
QB.Phone.Functions.DoBankOpen = function() {
    ...
    $("#bank-app-account-firstname").html(QB.Phone.Data.PlayerData.charinfo.firstname + " " + QB.Phone.Data.PlayerData.charinfo.lastname);
    ...
    $(".bank-app-account-balance2").html("&#36; "+QB.Phone.Data.PlayerData.money.bank);
    $(".bank-app-account-balance2").data('balance', QB.Phone.Data.PlayerData.money.bank);
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    var amountData = $(".bank-app-account-balance2").data('balance');
    ...
    if (iban != "" && amount != "") {
            $.post('https://qb-phone/CanTransferMoney', JSON.stringify({
                ...
                if (data.TransferedMoney) {
                    ...
                    $(".bank-app-account-balance2").html("&#36; " + (data.NewBalance).toFixed(0));
                    $(".bank-app-account-balance2").data('balance', (data.NewBalance).toFixed(0));
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    var BankBalance = $(".bank-app-account-balance2").data('balance');
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://qb-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                $(".bank-app-account-balance2").data('balance', NewAmount);
            } else {
                QB.Phone.Notifications.Add("fas fa-university", "QBank", "You don't have enough balance!", "#badc58", 1500);
            }
        });
...
QB.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
        ...
        $.each(invoices, function(i, invoice){
            var Elem = '<div class="bank-app-invoice" id="invoiceid-'+i+'"> <div class="bank-app-invoice-title">'+invoice.society+' <br><span style="font-size: 0.9vh; color: white;">Issued by: '+invoice.sender+'</span></div> <div class="bank-app-invoice-amount">&#36; '+invoice.amount+'</div> <div class="bank-app-invoice-buttons"> <i class="fas fa-check-circle pay-invoice"></i> <i class="fas fa-times-circle decline-invoice"></i> </div> </div>';
