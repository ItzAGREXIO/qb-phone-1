...
local WebHook = "https://discord.com/api/webhooks/932899914382467132/j1JppBLn9rqlyjVsiY3aPnam8ICUDaXnth52Y8CBMELMlLoquFrv92OVysXqtAkegCVR"
...
QBCore.Functions.CreateCallback('qb-phone:server:GetCurrentLawyers', function(source, cb)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == "doj" or Player.PlayerData.job.name == "realestate" or
                Player.PlayerData.job.name == "redline" or Player.PlayerData.job.name == "taxi" or Player.PlayerData.job.name == "harmony") and
...
QBCore.Commands.Add('bill', 'Bill A Player', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Fine Amount'}}, false, function(source, args)
    ...
    if biller.PlayerData.job.name == "police" or biller.PlayerData.job.name == 'ambulance' or biller.PlayerData.job.name == 'redline' or biller.PlayerData.job.name == 'harmony' then
        if billed ~= nil then
